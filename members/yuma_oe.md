...
- コンテキストを考慮したコーディネート生成のためのデータセット構築とベンチマーク

## 論文
1. **Asymmetric Pipeline for Dataset Construction and Situation-aware Generative Outfit Retrieval Leveraging Differences in Task Difficulty**  
著者: Yuma Oe (Shizuoka University), Katsumi Tanaka (Kwansei Gakuin University), Yoshiyuki Shoji (Shizuoka University)  
発表会議: The 16th ACM International Conference on Multimedia Retrieval (ICMR 2026), 2026

2. **Retrieval of LoRA Models based on Layer-Wise Weight Embedding without Metadata**  
...
3. **画像の生成とキャプショニングの粒度差に注目した生成的アノテーションによるシチュエーションからのコーディネート検索**
著者: 大江優真（静大）, 田中克己（関西学院大）, 莊司慶行（静大）
...
5. **ファッション生成画像の再ランキングのためのコーディネート画像の系統らしさの推定**  
著者: 大江優真（静大）, 莊司慶行（静大）   
発表会議: WebDB夏のワークショップ2025

6. **画風変換LoRAの内部パラメータによるモデルの埋め込み表現の獲得**  
著者: 金田悠路（静岡大）、大江優真（静岡大）、ファムフーロン（兵庫県立大）、加藤誠（筑波大）、大島裕明（兵庫県立大）、藤田澄男（LINEヤフー株式会社）、莊司慶行（静岡大）  
発表会議: WebDB夏のワークショップ2025

7. **Can Stable Diffusion Recommend Outfits?: Outfit Recommendation from Fashion Item Images via Generative AI**  
...
8. **ファッションアイテム画像をクエリとする画像生成AIを用いたコーディネート推薦**  
著者: 大江優真（静大）, 莊司慶行（静大）  
発表会議: 第17回データ工学と情報マネジメントに関するフォーラム, 2025

9. **スタイル距離計算に基づく画像を入力とする画像生成AIモデル検索**  
著者: 大江 優真（静大）, ブ・ティ・ゴッグ アン（静大）, ファム・フー・ロン（兵庫県立大）, 大島 裕明（兵庫県立大）, 莊司 慶行（静大）  
発表会議: 第17回データ工学と情報マネジメントに関するフォーラム, 2025

10. **Image-Generation AI Model Retrieval by Contrastive Learning-based Style Distance Calculation**  
...
11. **画像生成AIはファッションセンスを解すか？：投稿コーディネート画像を用いたLoRAモデルの作成と分析**  
著者: 大江優真（静大）, 莊司慶行（静大）  
...
- 株式会社ファーストリテイリング 勤続パートナーズ賞
- 第18回データ工学と情報マネジメントに関するフォーラム　最優秀インタラクティブ賞
